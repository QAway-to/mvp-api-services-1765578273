...
async function handleOrderUpdated(order) {
  ...
  if (financialStatusLower === 'partially_refunded') {
    ...
    console.log(`[SHOPIFY WEBHOOK] Financial status "${order.financial_status}" → Stage "${stageId}" (category ${categoryId}, current: "${currentStageId}")`);
    if (stageId !== currentStageId) {
      console.log(`[SHOPIFY WEBHOOK] ✅ Stage updated: "${currentStageId}" → "${stageId}"`);
    } else {
      console.log(`[SHOPIFY WEBHOOK] ℹ️ Stage unchanged: "${currentStageId}"`);
    }
  }
